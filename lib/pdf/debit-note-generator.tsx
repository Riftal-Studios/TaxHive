 ...
 * 
 * Generates GST-compliant debit note PDFs using React PDF.
 */
...
// Create styles (same as credit note)
...
interface DebitNoteWithRelations extends CreditDebitNote {
  invoice: Invoice & {
    client: Client;
    user: User;
  };
...
interface DebitNotePDFProps {
  debitNote: DebitNoteWithRelations;
}

// Debit Note PDF Component
export const DebitNotePDF: React.FC<DebitNotePDFProps> = ({ debitNote }) => {
  const { invoice } = debitNote;
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          <View style={styles.row}>
            ...
            <View style={styles.column}>
              <Text style={styles.label}>Debit Note Details:</Text>
              <Text style={styles.value}>Debit Note No: {debitNote.noteNumber}</Text>
              <Text style={styles.value}>Date: {formatDate(debitNote.noteDate)}</Text>
        ...
        <View style={styles.reasonBox}>
          <Text style={styles.label}>Reason for Debit Note:</Text>
        ...
        <View style={styles.totalsSection}>
          <View style={styles.totalRow}>
            ...
            <Text style={styles.totalValue}>{formatCurrency(debitNote.subtotal, invoice.currency)}</Text>
          </View>
          
          {/* Tax Amounts */}
          {debitNote.cgstAmount > 0 && (
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>CGST:</Text>
              <Text style={styles.totalValue}>{formatCurrency(debitNote.cgstAmount, invoice.currency)}</Text>
            </View>
          )}
          
          {debitNote.sgstAmount > 0 && (
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>SGST:</Text>
              <Text style={styles.totalValue}>{formatCurrency(debitNote.sgstAmount, invoice.currency)}</Text>
            </View>
          )}
          
          {debitNote.igstAmount > 0 && (
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>IGST:</Text>
              <Text style={styles.totalValue}>{formatCurrency(debitNote.igstAmount, invoice.currency)}</Text>
            </View>
          )}
          
          <View style={styles.grandTotalRow}>
            <Text style={styles.grandTotalLabel}>Total Debit:</Text>
            <Text style={styles.grandTotalValue}>{formatCurrency(debitNote.total, invoice.currency)}</Text>
          </View>
        </View>
        ...
        {debitNote.notes && (
          <View style={styles.section}>
            ...
            <Text style={styles.value}>{debitNote.notes}</Text>
          </View>
        )}
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>
            This is a computer-generated debit note and does not require a physical signature.
          </Text>
          <Text style={styles.footerText}>
...
// Generate PDF Buffer
export async function generateDebitNotePDF(debitNote: DebitNoteWithRelations): Promise<Buffer> {
  try {
    const pdfBuffer = await renderToBuffer(<DebitNotePDF debitNote={debitNote} />);
    return pdfBuffer;
  } catch (error) {
    console.error('Error generating debit note PDF:', error);
    throw new Error('Failed to generate debit note PDF');
  }
}
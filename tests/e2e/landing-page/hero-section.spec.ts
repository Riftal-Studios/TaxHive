...
test.describe("Landing Page - Hero Section", () => {
  ...
  test('should have H1 with "GST-Compliant Invoicing for Indian Freelancers"', async ({
    page,
  }) => {
  ...
  test("should have descriptive subtitle", async ({ page }) => {
    ...
    const subtitle = page
      .locator(
        "text=/Professional invoice management|Zero-rated supplies|export business/i",
      )
      .first();
  ...
  test("should have hero image with proper alt text", async ({ page }) => {
    ...
    const altText =
      (await heroImage.getAttribute("alt")) ||
      (await heroImage.getAttribute("aria-label"));
  ...
  test("hero section should have gradient or visual background", async ({
    page,
  }) => {

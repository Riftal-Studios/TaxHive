...
test.describe("Landing Page - SEO Meta Tags", () => {
  ...
  test('should have title tag with "TaxHive - GST-Compliant Invoice Management for Indian Freelancers"', async ({
    page,
  }) => {
    const title = await page.title();
    expect(title).toBe(
      "TaxHive - GST-Compliant Invoice Management for Indian Freelancers",
    );
  });

  test("should have meta description with 150-160 characters", async ({
    page,
  }) => {
  ...
  test("meta description should contain key terms (GST, invoice, export)", async ({
    page,
  }) => {
    const metaDescription = await page
      .locator('meta[name="description"]')
